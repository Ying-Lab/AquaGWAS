...
if ($vcfinput) {
	...
	$convertarg ||= '';
	$sc = "convert2annovar $convertarg -includeinfo -allsample -withfreq -format vcf4 $queryfile > $tempfile.avinput";
...
sub geneOperation {
	...
	#generate gene anno
	my $sc;
	$sc = "annotate_variation -geneanno -buildver $buildver -dbtype $protocol -outfile $tempfile.$protocol -exonsort -nofirstcodondel $queryfile $dbloc";		#20191010: add -nofirstcodondel
	...
	if ($polishgene) {
		if (rename ($e_anno_outfile, "$e_anno_outfile.orig")) {
			$codingarg ||= '';
			$sc = "coding_change $codingarg $e_anno_outfile.orig $dbloc/${buildver}_$protocol.txt $dbloc/${buildver}_${protocol}Mrna.fa -alltranscript -out $tempfile.$protocol.fa -newevf $e_anno_outfile";
			print STDERR "\nNOTICE: Running with system command <$sc>\n";
			system ($sc) and die "Error running system command: <$sc>\n";
